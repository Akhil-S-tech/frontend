...
import { updateDataFormGroupInServer } from "../../redux/actions/DataFormGroupAction";

function DataFormGroupUpdate(props) {
  const selectedDataFormGroup = useSelector((state) =>{
    return state.dataformgroup.dataFormGroupList});
  ...
  useEffect(() => {
     if (selectedDataFormGroup){

      setValues(selectedDataFormGroup);

    }
  }, [selectedDataFormGroup]); 

  const updateDataFormGroup= (event) => {
    event.preventDefault();
    props.onHide();
    dispatch(updateDataFormGroupInServer(values))
    navigate("/");  };
  return (
    <Modal
      {...props}
  ...
  <form onSubmit={updateDataFormGroup}>
		<div className="field">
...
export default DataFormGroupUpdate